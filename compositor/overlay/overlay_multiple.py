...
def match_hue_subtle(overlay_np, background_np):
    """
    Subtly adjust overlay hue to match background without affecting alpha.
    """
...
def paste_multiple_overlay(background: Image.Image, overlays: list, max_attempts: int = 20):
    """
    Paste all overlays onto background without overlaps.
    Returns composite image and YOLO labels.
    """
    ...
    for class_id, ov_img in enumerate(overlays):
        ov = ov_img.convert("RGBA")
        success = False

        for attempt in range(max_attempts):
            # Random scaling (20-40% of background width)
            ...
            ov_np = random_perspective_transform(ov_np, max_offset=0.1)
            # Subtle hue match
            ...
            if not check_overlap(new_box, placed_boxes):
                ...
                for c in range(3):
                    bg_np[y_min:y_min+new_h, x_min:x_min+new_w, c] = (
                        alpha * ov_np[..., c] + (1 - alpha) * bg_np[y_min:y_min+new_h, x_min:x_min+new_w, c]
                    )
                ...
                labels.append({"class_id": class_id, "bbox": (x_center, y_center, w_norm, h_norm)})
                success = True
                break

        if not success:
            print(f"Warning: Could not place overlay {class_id} after {max_attempts} attempts. Skipping.")

    composite = Image.fromarray(bg_np).convert("RGB")  # Convert to RGB for JPEG saving
